		...
		if (timeSinceLastTap < doubleTapDelay.value) {
			...
			if (tapCount.value >= 2) {
				if (cardRef.value) {
					...
					setTimeout(() => {
						cardRef.value && cardRef.value.classList.remove("double-tap-feedback");
					}, 300);
				}
				flipCard();
				tapCount.value = 0;
...
onMounted(async () => {
	...
	const handleGlobalMouseUp = () => {
		if (isMouseDown.value) handleMouseUp();
	};
...
.profile-card::after {
	...
	background: url("https://assets.codepen.io/13471/sparkles.gif"), url("https://assets.codepen.io/13471/holo.png"), linear-gradient(115deg, var(--c1), var(--c2));
...
.card-back::before {
	...
	background: radial-gradient(
			...
			rgba(255, 255, 255, calc(var(--shineO) * 0.6)) 16%,
			rgba(255, 255, 255, calc(var(--shineO) * 0.25)) 32%,
			rgba(255, 255, 255, 0) 60%
		),
		linear-gradient(135deg, rgba(255, 255, 255, calc(var(--shineO) * 0.3)) 0%, rgba(255, 255, 255, 0) 60%);
...
.profile-header::after {
	...
	opacity: calc(var(--opc) * var(--grade-gloss));
	filter: brightness(calc(1 + 0.25 * var(--grade-gloss))) contrast(calc(1 + 0.15 * var(--grade-gloss)));
	background: url("https://assets.codepen.io/13471/sparkles.gif"), url("https://assets.codepen.io/13471/holo.png"),
		linear-gradient(125deg, #ff008450 15%, #fca40040 30%, #ffff0030 40%, #00ff8a20 60%, #00cfff40 70%, #cc4cfa50 85%);
	background-size: 160%;
	background-position: var(--px_s) var(--py_s);
...
@media (max-width: 440px) {
	...
	.main-content {
		...
		padding-bottom: calc(var(--bottom-nav-height) + 16px + constant(safe-area-inset-bottom, 0px)) !important;
	}
}
