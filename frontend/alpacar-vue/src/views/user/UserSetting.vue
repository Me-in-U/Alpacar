<template>
	<div class="user-setting">
		...
		<!-- Back to UserProfile -->
		<button class="back-link" type="button" @click="goToUserProfile" aria-label="프로필로 돌아가기">
		...
		<div class="user-setting__content">
			...
			<div class="profile-card">
				<!-- 전화번호 행 -->
				<button class="setting-row" type="button" @click="openPhoneModal">
					<div class="setting-row__text">
						<div class="setting-row__label">전화번호</div>
				...
				<button class="setting-row" type="button" @click="openPasswordModal">
					<div class="setting-row__text">
						<div class="setting-row__label">비밀번호</div>
						<div class="setting-row__value setting-row__value--placeholder">변경하기</div>
					</div>
					<span class="chevron" aria-hidden="true">
						<svg viewBox="0 0 24 24">
							<path d="M9 6l6 6-6 6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
		...
		<div v-if="showPhoneModal" class="modal-overlay" @click="closePhoneModal">
			<div class="modal modal--phone" @click.stop>
				<!-- X Close Button -->
				...
				<h3 class="modal__title">새 전화번호 입력</h3>
				...
				<div v-if="newPhoneNumber && !isPhoneValid" class="error-message">010, 011, 016, 017, 018, 019로 시작하는 10~11자리 번호만 입력 가능</div>

				<button class="modal__button" @click="requestPhoneChange" :disabled="!isPhoneValid">변경하기</button>
			</div>
		</div>
		...
		<div v-if="showPasswordModal" class="modal-overlay" @click="showPasswordModal = false">
			<div class="modal modal--password" @click.stop>
				...
				<h3 class="modal__title">비밀번호 변경</h3>

				<div class="section-subtitle">현재 비밀번호 입력</div>
				<div class="modal__input-field">
					<input v-model="currentPassword" type="password" placeholder="현재 비밀번호를 입력하세요" class="modal__input" maxlength="20" />
				</div>

				<div class="section-subtitle">새 비밀번호 입력</div>
				<div class="modal__input-field">
					<input v-model="newPassword" type="password" placeholder="새 비밀번호를 입력하세요" class="modal__input" maxlength="20" />
				</div>
				...
				<div class="modal__input-field">
					<input v-model="confirmPassword" type="password" placeholder="새 비밀번호를 다시 입력하세요" class="modal__input" maxlength="20" />
				</div>

				<div v-if="confirmPassword && !isPasswordConfirmValid" class="error-message">비밀번호가 일치하지 않습니다</div>

				<button class="modal__button" @click="requestPasswordChange">변경하기</button>
			</div>
		</div>
		...
		<div v-if="showEmailVerificationModal" class="modal-overlay" @click="showEmailVerificationModal = false">
			<div class="modal modal--email-verify" @click.stop>
				...
				<h3 class="modal__title">{{ verificationTarget === "phone" ? "전화번호 변경" : "비밀번호 변경" }} 인증</h3>
				...
				<div class="verification-step">
					<button class="modal__button" @click="sendEmailVerification" :disabled="emailSent">
				...
				<div v-if="emailSent" class="verification-input">
					<div class="modal__input-field">
						<input v-model="verificationCode" type="text" placeholder="인증번호를 입력하세요" class="modal__input" maxlength="6" />
					</div>

					<button class="modal__button" @click="verifyEmailCode" :disabled="!verificationCode || emailVerified">
						{{ emailVerified ? "인증완료" : "인증확인" }}
					</button>
				</div>

				<div v-if="emailVerified" class="verification-complete">
					<button class="modal__button modal__button--success" @click="verificationTarget === 'phone' ? executePhoneChange() : confirmPasswordChange()">
...
const isPhoneValid = computed(() => {
	...
	return /^(010|011|016|017|018|019)\d{7,8}$/.test(newPhoneNumber.value);
});
...
const preventInvalidPhoneChars = (e: KeyboardEvent) => {
	const char = e.key;
	if (!/\d/.test(char) && !["Backspace", "Delete", "ArrowLeft", "ArrowRight", "Tab"].includes(char)) e.preventDefault();
};
...
const executePhoneChange = async () => {
	if (!emailVerified.value) {
		await alertWarning("이메일 인증을 먼저 완료해주세요.");
		return;
	}
	if (!newPhoneNumber.value || !isPhoneValid.value) {
		await alertWarning("올바른 전화번호를 입력해주세요.");
		return;
	}
...
const isPasswordValid = computed(() => [passwordLengthValid, passwordLetterValid, passwordNumberValid, passwordSpecialValid, passwordNoTripleValid, passwordNoSeqValid].every((v) => v.value));
