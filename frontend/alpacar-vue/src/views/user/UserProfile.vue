  ...
  <div class="user-profile">
    ...
    <div v-if="showVehicleModal" class="modal-overlay" @click="showVehicleModal = false">
      <div class="modal modal--vehicle" @click.stop>
        ...
        <button class="modal__button" @click="addSimpleVehicle" :disabled="!canAddVehicle">
          등록완료
        </button>
      </div>
    </div>
...
const checkVehicleDuplicate = async () => {
  ...
  try {
    const response = await fetch(
      `${BACKEND_BASE_URL}/vehicles/check-license/?license=${encodeURIComponent(vehicleNumber.value)}`,
      {
        method: "GET",
        headers: {
    ...
    console.error(e);
    alert("차량번호 중복체크 중 오류가 발생했습니다. (테스트 모드)");
    vehicleChecked.value = false;
    isDuplicate.value = false;
  }
};
...
const addSimpleVehicle = async () => {
  ...
  try {
    const response = await fetch(`${BACKEND_BASE_URL}/vehicles/`, {
      ...
      body: JSON.stringify({ license_plate: vehicleNumber.value, model: 1 })
    });
    if (response.ok) {
      alert("차량이 성공적으로 등록되었습니다. (테스트)");
    ...
    console.error(e);
    alert("차량 등록 중 오류가 발생했습니다. (테스트 모드)");
  }
};
...
.user-info__item:hover { background-color: rgba(119, 107, 93, 0.02); }

.user-info__icon-wrapper { margin-right: 16px; flex-shrink: 0; }
