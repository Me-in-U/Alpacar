  ...
  notificationStore.notifications = [
    {
      id: 1,
      title: "주차 배정 안내",
      message: "A3 구역에 주차 자리가 배정되었습니다.",
      is_read: false,
      notification_type: "parking",
      created_at: new Date().toISOString(),
      data: { parking_time: new Date().toISOString(), parking_space: "A3" }
    },
    {
      id: 2,
      title: "주차 완료",
      message: "B1 구역 주차가 완료되었습니다.",
      is_read: true,
      notification_type: "parking_complete",
      created_at: new Date(Date.now() - 3600000).toISOString(),
      data: { parking_time: new Date(Date.now() - 3600000).toISOString(), parking_space: "B1" }
    },

  ];
  notificationStore.hasMore = false; // 더보기 버튼 안보이게
...
.delete-all:hover { background-color: rgba(0, 0, 0, 0.1); }
...
.notification-message { margin-top: 5px; }

.notification-item.unread {
	border-left: 4px solid #212730; /* 변경 */
	background: #f0ede8;
}
...
.load-more-button:hover:not(:disabled) { background: #272d37; /* 변경 */ }
...
.delete-button:hover { background-color: rgba(0, 0, 0, 0.1); }
...
.close-button:hover {
	background-color: #272d37; /* 변경 */
	transform: translateY(-2px);
	box-shadow: 0 4px 12px rgba(33, 39, 48, 0.3); /* 변경 */
}
...
@media (max-width: 440px) {
    ...
    height: calc(100vh - 160px); /* 헤더(80px) + 하단 네비게이션바(80px) 제외 */
		border-radius: 0; margin-top: 80px; /* 헤더 높이만큼 상단 마진 */
