...
class ParkingStatus:
    ...
    def __init__(self):
        self.parked_vehicles = (
            {}
        )  # {track_id: {'start_time': timestamp, 'duration': seconds, 'position': (x, y)}}
    ...
    def update_vehicle_status(self, track_id, position, timestamp):
        ...
        if track_id not in self.parked_vehicles:
            ...
            if distance < self.movement_threshold:
                ...
                if (
                    vehicle["duration"] >= self.parking_threshold
                    and not vehicle["is_parked"]
                ):
    ...
    def get_parked_vehicles_count(self):
        """주차된 차량 수 반환"""
        return sum(
            1 for vehicle in self.parked_vehicles.values() if vehicle["is_parked"]
        )
...
class EnhancedVideoProcessor:
    ...
    def adjust_brightness(self, frame):
        """프레임의 밝기를 조절"""
        # 기존: if self.brightness_adjustment != 0:
        if abs(self.brightness_adjustment) > FLOAT_EPS:
            frame = cv2.convertScaleAbs(frame, alpha=1, beta=self.brightness_adjustment)

        # 기존: if self.contrast_adjustment != 1.0:
        if abs(self.contrast_adjustment - 1.0) > FLOAT_EPS:
            frame = cv2.convertScaleAbs(frame, alpha=self.contrast_adjustment, beta=0)

        # 기존: if self.gamma_adjustment != 1.0:
        if abs(self.gamma_adjustment - 1.0) > FLOAT_EPS:
            gamma_table = np.array(
                [
                    ((i / 255.0) ** (1.0 / self.gamma_adjustment)) * 255
    ...
    def draw_enhanced_box(self, frame, xyxy, track_id, class_name, confidence, color):
        ...
        cv2.rectangle(
            frame, (rect_left, label_top), (rect_right, label_bottom), color, -1
        )
    ...
    def draw_track_history(self, frame, track_id, current_pos):
        ...
        for i in range(1, len(recent_history)):
            if recent_history[i - 1] is not None and recent_history[i] is not None:
                cv2.line(
                    frame, recent_history[i - 1], recent_history[i], (0, 255, 0), 2
                )
    ...
    def create_kick_off_frame(self, width, height, fps, total_frames):
        ...
        cv2.rectangle(
            frame,
            (title_x - 20, title_y - title_size[1] - 20),
            (title_x + title_size[0] + 20, title_y + 20),
            (50, 50, 50),
            -1,
        )
        cv2.putText(
    ...
    def process_video_enhanced(self, source_path="WIN_20250807_10_08_53_Pro.mp4"):
        ...
        print(
            f"비디오 정보 - 해상도: {width}x{height}, FPS: {fps}, 총 프레임: {total_frames}"
        )
        ...
        try:
            while True:
                ...
                if frame_count % 30 == 0:
                    ...
                    print(
                        f"처리 진행률: {progress:.1f}% ({frame_count}/{total_frames})"
                    )
    ...
    def process_obb_results(self, frame, result, current_time):
        ...
        for i in range(len(xyxyxyxy)):
            ...
            # 주차 상태 업데이트
            self.parking_status.update_vehicle_status(
