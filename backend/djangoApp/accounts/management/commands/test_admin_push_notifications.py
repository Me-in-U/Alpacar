...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'사용자를 찾을 수 없습니다: {email}')
            )
            return

        self.stdout.write(f"=== 관리자 이벤트 푸시 알림 테스트 ===")
    ...
    def test_parking_assignment_notification(self, user):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f'   ❌ 주차 배정 알림 전송 실패: {str(e)}'))
    ...
    def test_parking_reassignment_notification(self, user):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f'   ❌ 주차 재배정 알림 전송 실패: {str(e)}'))
    ...
    def test_parking_complete_notification(self, user):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f'   ❌ 주차 완료 알림 전송 실패: {str(e)}'))
    ...
    def test_vehicle_exit_notification(self, user):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f'   ❌ 출차 완료 알림 전송 실패: {str(e)}'))