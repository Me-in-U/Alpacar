...
from vehicles.models import VehicleLicensePlateModelMapping, Vehicle
...
if VehicleLicensePlateModelMapping.objects.count() > 0:
    ...
    # 테스트용 라이센스 플레이트 찾기
    test_plates = ['12가3456', '123가4567', '01가1234', '서울12가3456']
    print('\n=== 테스트 라이센스 플레이트 검사 ===')
    for plate in test_plates:
        exists = VehicleLicensePlateModelMapping.objects.filter(license_plate=plate).exists()
...
if Vehicle.objects.count() > 0:
    ...
    for i, vehicle in enumerate(Vehicle.objects.all()[:5], 1):
        print(f'{i}. {vehicle.license_plate} -> user: {vehicle.user}, model: {vehicle.model}')