...
for mapping in VehicleLicensePlateModelMapping.objects.all():
    ...
    print(f'  - Characters: {[c for c in plate]}')
    print('---')
...
# 데이터베이스에서 직접 검색
found = VehicleLicensePlateModelMapping.objects.filter(license_plate=test_plate).exists()
print(f'  - Found in DB: {found}')

# 유사한 패턴 검색
similar = VehicleLicensePlateModelMapping.objects.filter(license_plate__contains='가').values_list('license_plate', flat=True)
