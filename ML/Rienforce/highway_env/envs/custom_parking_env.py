...
class CustomParkingEnv(AbstractEnv, GoalEnv):
        ...
        config.update(
            {
                ...
                # [SmallVehicle, MediumVehicle] 확률
                "vehicle_type_probabilities": [0.5, 0.5],
                "controller_mode": "random", # random, basic
                # lane 길이가 임계값 미만일 경우 SmallVehicle만 배치
                # 0 또는 음수면 비활성화
                "short_lane_length_threshold": 5,
            }
        )
        return config
    ...
    def _create_road(self, spots: int = 6) -> None:
        ...
        x_offset = 0.0  # lane 간격을 0에서 6.0으로 확장
        y_offset = 6
        length = 5.5
        x_shift = 10
        for k in range(3):
        ...
        # 위쪽에 하나 더 생성
        x = x_shift + (spots - 2 - spots // 2) * (width + x_offset) - width / 2 + 2
        net.add_lane(
                "b",
                "c",
                StraightLane(
                    [x, -y_offset - 0.5], [x, -y_offset - (length - 1)], width=width, line_types=lt
                ),
            )
        x = x_shift + (spots - 1 - spots // 2) * (width + x_offset) - width / 2 + 2
        net.add_lane(
                "b",
                "c",
                StraightLane(
        ...
        x = x_shift + (spots - spots // 2) * (width + x_offset) - width / 2 + 2
        net.add_lane(
                "b",
                "c",
                StraightLane(
    ...
    def _get_random_vehicle_class(self):
        ...
        # Small, Medium만 사용
        vehicle_classes = [SmallVehicle, MediumVehicle]
    ...
    def _create_vehicles(self) -> None:
        ...
        for i in range(self.config["controlled_vehicles"]):
            x0 = (i - self.config["controlled_vehicles"] // 2) * 9 - self.np_random.uniform(2, 10)
            # 제어 차량도 Small 또는 Medium만 생성
            ctrl_class = self._get_random_vehicle_class()
            heading = 2 * np.pi * self.np_random.uniform() if self.config["controller_mode"] == "random" else 0
            vehicle = ctrl_class(self.road, [x0, 0], heading, 0)
            # 설정에서 방향 오프셋 적용
        ...
        for i in range(self.config["vehicles_count"]):
            ...
            # lane 길이에 따른 차량 클래스 선택
            lane_for_spawn = self.road.network.get_lane(lane_index)
            threshold = self.config.get("short_lane_length_threshold", 0.0)
            if isinstance(threshold, (int, float)) and threshold > 0 and lane_for_spawn.length < threshold:
                vehicle_class = SmallVehicle
            else:
                vehicle_class = self._get_random_vehicle_class()
            v = vehicle_class.make_on_lane(self.road, lane_index, 2, speed=0)
    ...
    def _create_parked_vehicles(self):
        ...
        parked_positions = [
            # 좌측 주차된 차량들
            ([i, -7], np.pi / 2) for i in range(-27, -2, 2)
        ] + [
            # 우측 주차된 차량들  
            ([i, 7], np.pi / 2) for i in range(-27, -2, 2)
        ] + [
    ...
    def _calculate_tiered_angle_score(self, angle_diff):
        """
        3단계 각도 평가 시스템
    ...
    def get_agent_angle_score(self, vehicle: Vehicle):
        """
        에이전트 차량의 주차 각도 점수 계산
        """
        if vehicle.goal is None:
            return {
                "agent_angle_score": 0,
                "agent_angle_tier": "none",
                "agent_angle_deg": 0
            }
        
        # goal lane 찾기
        goal_lane = None
        for lane in self.road.network.lanes_dict().values():
            if np.allclose(lane.position(lane.length/2, 0), vehicle.goal.position, atol=0.1):
                goal_lane = lane
                break
        ...
        # 에이전트 차량의 각도 점수 계산
        agent_angle_diff = abs(vehicle.heading - goal_lane.heading)
        agent_angle_deg = np.rad2deg(agent_angle_diff)
        agent_angle_score = self._calculate_tiered_angle_score(agent_angle_deg)
        
        # 에이전트 차량의 각도 등급 분류
        if agent_angle_deg <= 5:
            agent_angle_tier = "expert"  # 상급자
        elif agent_angle_deg <= 10:
            agent_angle_tier = "intermediate"  # 중급자
        else:
            agent_angle_tier = "beginner"  # 초급자

        return {
            "agent_angle_score": agent_angle_score,
            "agent_angle_tier": agent_angle_tier,
            "agent_angle_deg": agent_angle_deg
        }
    ...
    def extract_goal_features(self, vehicle: Vehicle):
        ...
        return {
            ...
            **goal_info,
            "controlled_x": vehicle.position[0],
            "controlled_y": vehicle.position[1],
            "controlled_width": getattr(vehicle, "WIDTH", None),
            "controlled_length": getattr(vehicle, "LENGTH", None),
        }
