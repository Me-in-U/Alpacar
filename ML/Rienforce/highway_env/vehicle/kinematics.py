...
class SmallVehicle(Vehicle):
    ...
    작은 크기와 민첩한 조작성을 특징으로 하는 소형 차량입니다.
    """
    ...
    def __init__(
        ...
        self.vehicle_type = "small"
        # SmallVehicle은 시각적으로 구분되도록 보라색 계열 색상을 사용
        # VehicleGraphics.PURPLE 과 동일값: (200, 0, 150)
        self.color = (200, 0, 150)
        # 소형차는 더 민첩한 조작을 위해 조향 반응성 향상
        self.steering_sensitivity = 1.2
    
    def step(self, dt: float) -> None:
        """
        소형차의 특성을 반영한 상태 업데이트
        
        소형차는 더 민첩한 조향 반응을 가집니다.
        """
        # 조향 감도 적용
        original_steering = self.action["steering"]
        self.action["steering"] *= self.steering_sensitivity
        super().step(dt)
        self.action["steering"] = original_steering
...
class MediumVehicle(Vehicle):
    ...
    중간 크기와 균형잡힌 성능을 특징으로 하는 중형 차량입니다.
    """
    ...
    def __init__(
        ...
        super().__init__(road, position, heading, speed, predition_type)
        self.vehicle_type = "medium"
        # 중형차는 안정적인 주행을 위해 조향 반응성 조정
        self.steering_sensitivity = 0.9
    
    def step(self, dt: float) -> None:
        """
        중형차의 특성을 반영한 상태 업데이트
        
        중형차는 더 안정적인 조향 반응을 가집니다.
        """
        # 조향 감도 적용
        original_steering = self.action["steering"]
        self.action["steering"] *= self.steering_sensitivity
