...
1. [유니티 설치](https://unity.com/kr/download)
...
5. 객체 생성 스크립트 작성
   에셋에서 마우스 우클릭 -> Create -> Scripting -> Empty C# Scripting 으로 스크립트 생성
   소스는 [링크 참조](../ML/ImageDetection/ObjectSpanwer.cs)
   ![alt text](imgs/image-4.png)

6. 스크립트 적용
   Hierachy 탭에서 우클릭 -> Create Empty 로 Spawner 생성  
   이후 스크립트를 Add Component로 추가
   ![alt text](imgs/image-5.png)

7. 타일맵 작성 및 데이터셋 생성
   원하는 상황에 따른 디자인 적용  
   아래 사진처럼 Car Models에 에셋들을 추가하고 파라미터를 설정하여 실행하면 데이터가 생성됌
...
2. 시뮬레이션 장면 생성
   - 프로젝트 탭에서 assert 폴더에 마우스 우클릭을 통해 scenes 폴더 생성
   - 생성된 폴더에 마우스 우클릭을 통해 Create → Scenes 를 클릭
   - 새로 생긴 Scene을 더블클릭하여 진입
     ![alt text](imgs/장면생성.png)
3. Main Camera 설정

   화면 좌측 상단의 Main Camera를 클릭하여 우측 Inspector 탭에서 아래와 같은 상태인지 확인
   ![alt text](imgs/메인카메라.png)

4. Perception 적용

   하단의 Add Component에서 Perception Camera 추가
...
6. Ground Truth 라벨러 설정
   ...
   Assets 에서 마우스 우클릭 → Create → Perception → ID Label Config 로 라벨러 생성
...
9. 랜더마이저 설정
   ...
   화면 좌측 Hierarchy 탭에서 + 버튼 또는 마우스 우클릭으로 Create Empty 로 GameObject 생성
   ...
   Depth = 0, Layer Count = 2, Separation Distance = 0.5, Placement Area = (6,6) 지정
   ![alt text](imgs/백그라운드.png)
   TextureRandomizer, HueOffsetRandomizer, RotationRandomizer 등을 추가해서 추가적인 변수 생성

   이후 ForeGroundObjectPlacementRandomizer 도 같은 방식으로 추가 후 Depth = -3, Separation Distance = 1.5, Placement Area = (5, 5) 로 지정
